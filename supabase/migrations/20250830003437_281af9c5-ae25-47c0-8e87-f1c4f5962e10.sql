...
-- Create some debates using correct debate types
...
VALUES 
  ('22222222-2222-2222-2222-222222222222', 'Pineapple on Pizza: Culinary Crime or Misunderstood Masterpiece?', 'The age-old debate that has torn families apart and ended friendships. Time to settle this once and for all!', 'text', 1000, 0.005, 4, 'scheduled', NOW() + INTERVAL '2 hours', ARRAY['No personal attacks', 'Stick to food arguments', '5 minute rounds', 'No bringing up other controversial toppings']),
  ('33333333-3333-3333-3333-333333333333', 'Is Social Media Making Us More Connected or More Isolated?', 'A deep dive into the paradox of modern digital communication and its impact on human relationships.', 'video', 2500, 0.01, 6, 'scheduled', NOW() + INTERVAL '1 day', ARRAY['Academic sources preferred', 'No anecdotal evidence only', '10 minute opening statements', 'Respectful discourse required']),
  ('44444444-4444-4444-4444-444444444444', 'Cats vs Dogs: The Ultimate Pet Showdown', 'Which furry friend reigns supreme? Prepare your arguments and may the best species win!', 'audio', 500, 0.002, 8, 'live', NOW() - INTERVAL '30 minutes', ARRAY['Photos allowed as evidence', 'No bringing up allergies', 'Cute factor counts', 'Must own or have owned the pet you''re defending']);
...
SELECT 
  ...
  id,
  'Absolutely brutal! 😂 But also 100% accurate!',
  8
FROM roasts WHERE content LIKE '%influencer%' LIMIT 1;

INSERT INTO comments (author_id, roast_id, content, likes_count)
SELECT 
  '33333333-3333-3333-3333-333333333333',
  id,
  'This hits different when you have 11 followers... asking for a friend 👀',
  12
FROM roasts WHERE content LIKE '%influencer%' LIMIT 1;
...
SELECT 
  ...
  id,
  'Philosophy majors in shambles right now 📚💔',
  5
FROM roasts WHERE content LIKE '%money%' LIMIT 1;
