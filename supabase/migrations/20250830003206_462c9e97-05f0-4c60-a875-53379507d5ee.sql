-- Set up storage policies for user avatars and content images
INSERT INTO storage.buckets (id, name, public, file_size_limit, allowed_mime_types) 
VALUES 
  ('avatars', 'avatars', true, 5242880, ARRAY['image/jpeg', 'image/png', 'image/webp', 'image/gif']),
  ('roast-media', 'roast-media', true, 10485760, ARRAY['image/jpeg', 'image/png', 'image/webp', 'image/gif', 'video/mp4', 'video/webm'])
...
-- Create storage policies for roast media
CREATE POLICY "Roast media is publicly accessible" 
ON storage.objects 
FOR SELECT 
USING (bucket_id = 'roast-media');

CREATE POLICY "Users can upload roast media" 
ON storage.objects 
FOR INSERT 
WITH CHECK (bucket_id = 'roast-media' AND auth.uid()::text = (storage.foldername(name))[1]);

CREATE POLICY "Users can update their own roast media" 
ON storage.objects 
FOR UPDATE 
USING (bucket_id = 'roast-media' AND auth.uid()::text = (storage.foldername(name))[1]);

CREATE POLICY "Users can delete their own roast media" 
ON storage.objects 
FOR DELETE 
USING (bucket_id = 'roast-media' AND auth.uid()::text = (storage.foldername(name))[1]);
